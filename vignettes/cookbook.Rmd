...
- We also need to update the output directories for each of the runs to ensure the results of one configuration are not overwritten with the results of the other when rerunning the functions.
  ...
  - For `analyse()`, we set the output to something like: `dir_analysis: "path/to/analysis_aggregate"` and in the second run to `dir_analysis: "path/to/analysis_bank_type"`. We also need to make sure that when we run analysis(), the config.yml file points to the correct output directories for both the `prioritized_loanbooks_and_diagnostics` and `analysis` functions, so the correct input files can be accessed.
...
- We also need to update the output directories for each of the runs to ensure the results of one configuration are not overwritten with the results of the other when rerunning the functions.
  ...
  - For `analyse()`, we set the output to something like: `dir_analysis: "path/to/analysis_group_id"` and in the second run to `dir_analysis: "path/to/analysis_net_zero_pledge"`. We also need to make sure that when we run analysis(), the config.yml file points to the correct output directories for both the `prioritized_loanbooks_and_diagnostics` and `analysis` functions, so the correct input files can be accessed.
